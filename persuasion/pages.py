...
class Receiver_guess_no(Page):
    form_model = 'group'
    form_fields = ['red_guess','blue_guess']

    def is_displayed(self):
        return (self.player.id_in_group == 2) and (self.group.observation == 0)
...
class Receiver_guess_o1(Page):
    ...
    def is_displayed(self):
        return (self.player.id_in_group == 2) and (self.group.observation == 1) and (self.group.true_type == 1)
...
class Receiver_guess_o2(Page):
    ...
    def is_displayed(self):
        return (self.player.id_in_group == 2) and (self.group.observation == 1) and (self.group.true_type == 0)
    ...
    def after_all_players_arrive(self):
        ...
        if self.group.message == 1:
            if self.group.red_guess == self.group.true_state:
                p2.round_payoff = Constants.receiver_payoffs[1] - self.group.observation*Constants.observation_cost
            else:
                p2.round_payoff = Constants.receiver_payoffs[0] - self.group.observation*Constants.observation_cost
...
page_sequence = [init_calc, Sender_instruction, Receiver_instruction, Sender_decision, WaitforP1, Receiver_observe, Receiver_guess_no, Receiver_guess_o1,Receiver_guess_o2,Waitforall, Results]
